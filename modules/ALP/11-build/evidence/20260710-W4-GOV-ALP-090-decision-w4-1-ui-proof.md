...
|---|---|---|
| Enrolled learner allowed | A learner with `course_enrolments.status = enrolled` can access `/learn/[courseSlug]` and `/learn/[courseSlug]/units/[unitSlug]`. | Pending capture |
| Not enrolled denied | A learner with no enrolment row receives the governed access-denied state and does not see gated course content. | Pending capture |
| Pending status denied | A learner with `course_enrolments.status = pending` receives the governed access-denied state and does not see gated course content. | Pending capture |
| Revoked status denied | A learner with `course_enrolments.status = revoked` receives the governed access-denied state and does not see gated course content. | Pending capture |
| Unknown/error denied | An unresolved enrolment access decision does not expose gated course content. | Pending capture |
