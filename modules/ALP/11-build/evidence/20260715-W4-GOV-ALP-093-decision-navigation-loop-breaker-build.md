...
|---|---|
| FR-ALP-NAV-001 | Added persistent `LearnerSidebar` for signed-in learner sessions. |
...
| File | Purpose |
|---|---|
| `src/components/navigation/LearnerSidebar.tsx` | Persistent signed-in learner navigation and route labels. |
| `app/layout.tsx` | Session-aware learner shell; removes footer-only navigation dependency. |
| `app/navigation-shell.css` | Responsive sidebar/shell styling. |
| `src/components/course/CourseAccessDenied.tsx` | Denied-state recovery routes and sign-out control. |
...
| QA-ALP-249 | Root layout renders a session-aware learner shell. |
| QA-ALP-250 | Denied state exposes recovery navigation and sign-out. |
| QA-ALP-251 | Footer-only route navigation is removed and the sidebar is responsive. |
